    ...
    def clean_password1(self):
        ...
        if(len(password) < 10):
            raise ValidationError(
                'Please enter at least 10 characters of strings')
        if(password.isalnum()):
            raise ValidationError(
                'Please enter at least one character that is not alphabet or number')
        return password
...
class CustomUpdateForm(forms.ModelForm):
    ...
    class Meta:
        model = UserModel
        include = "__all__"
        exclude = ('date', 'updated_time', 'bmi', 'height', 'weight',
                   'user', 'email_confirmed',  'state')
